...
p = Path(r"src/SmartInvoice.Application/Services/IBackgroundJobService.cs")
content = r'''using System.Collections.Generic;
    ...
    string? FailureSummaryJson = null)
{
    ...
    public string ReportSummary
    {
        get
        {
            ...
            AppendFailureIdHint(reportLines);
            return string.Join(Environment.NewLine, reportLines);
        }
    }

    private void AppendFailureIdHint(List<string> lines)
    {
        var s = JobFailureSummary.Parse(FailureSummaryJson);
        if (s.DetailFailedIds.Count > 0)
            lines.Add($"• Hóa đơn lỗi chi tiết (có thể chạy lại): {s.DetailFailedIds.Count}");
        if (s.XmlFailedIds.Count > 0)
            lines.Add($"• Hóa đơn lỗi XML (có thể chạy lại): {s.XmlFailedIds.Count}");
        if (s.PdfFailedIds.Count > 0)
            lines.Add($"• Hóa đơn lỗi PDF (có thể chạy lại): {s.PdfFailedIds.Count}");
    }
}
...
print("written", p, "utf8 ok", "Đ" in content)
